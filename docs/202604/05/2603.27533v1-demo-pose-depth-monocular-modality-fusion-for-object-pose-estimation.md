...
## 摘要
物体位姿估计是3D视觉中的一项基础任务，在机器人、增强现实/虚拟现实（AR/VR）以及场景理解中具有广泛应用。我们解决了基于RGB-D输入的类别级9自由度（9-DoF）位姿估计（6D位姿 + 3D尺寸）的挑战，且在推理过程中不依赖CAD模型。现有的仅依赖深度的方法虽然取得了显著成果，但忽略了来自RGB图像的语义线索；而许多RGB-D融合模型由于跨模态融合效果欠佳，无法将语义RGB线索与3D几何表示有效对齐，导致性能受限。我们提出了DeMo-Pose，这是一种混合架构，通过一种新颖的多模态融合策略，将单目语义特征与基于深度的图卷积表示相结合。为了进一步提升几何推理能力，我们引入了一种新颖的网格-点损失（Mesh-Point Loss, MPL），该损失在训练过程中利用网格结构，且不会增加推理开销。我们的方法实现了实时推理，并在各类物体上显著优于现有最先进的方法，在REAL275基准测试中，其3D IoU和位姿准确度分别比强基准模型GPV-Pose高出3.2%和11.1%。实验结果突显了深度-RGB融合与几何感知学习的有效性，为现实应用中的鲁棒类别级3D位姿估计提供了可能。
