...
tldr: OMNI-PoseX 是一款针对具身智能任务设计的快速 6D 物体位姿估计视觉基础模型。针对开放世界中现有方法泛化性差和实时性不足的问题，该模型结合了开放词汇感知与 SO(3) 感知的反射流匹配预测器，通过解耦物体理解与几何一致性旋转推理，实现了高效稳定的位姿估计。在大规模数据集上的训练使其具备强大的零样本泛化能力，在机器人抓取任务中表现出色。
selection_source: fresh_fetch
motivation: 现有的 6D 位姿估计方法多依赖闭集假设或忽略几何一致性，难以满足开放世界机器人任务对泛化性和实时性的需求。
method: 提出一种统一开放词汇感知与 SO(3) 感知反射流匹配的架构，通过轻量化多模态融合策略将语义嵌入与几何特征结合。
result: 在多个基准测试中达到 SOTA 精度，具备实时运行效率，并成功应用于未知物体的机器人可靠抓取。
conclusion: OMNI-PoseX 证明了结合开放词汇语义与几何感知流匹配能有效提升 6D 位姿估计在复杂具身环境中的鲁棒性与通用性。
---

## 摘要
精确的 6D 物体位姿估计是具身智能体的基本能力，但在开放世界环境中仍极具挑战性。许多现有方法通常依赖于闭集假设或与几何无关的回归方案，限制了它们在机器人系统中的泛化性、稳定性和实时适用性。我们提出了 OMNI-PoseX，这是一种视觉基础模型，它引入了一种新颖的网络架构，将开放词汇感知与感知 SO(3) 的反射流匹配（reflected flow matching）位姿预测器相统一。该架构将物体级理解与几何一致的旋转推理相解耦，并采用轻量级多模态融合策略，将旋转敏感的几何特征以紧凑的语义嵌入为条件，从而实现高效且稳定的 6D 位姿估计。为了增强鲁棒性和泛化性，该模型在大型 6D 位姿数据集上进行训练，利用广泛的物体多样性、视角变化和场景复杂性，构建了一个可扩展的开放世界位姿骨干网络。在基准位姿估计、消融研究、零样本泛化以及系统级机器人抓取集成方面的全面评估证明了 OMNI-PoseX 的有效性。OMNI-PoseX 实现了 SOTA 级的位姿准确度和实时效率，同时提供几何一致的预测，从而能够可靠地抓取各种先前未见的物体。
