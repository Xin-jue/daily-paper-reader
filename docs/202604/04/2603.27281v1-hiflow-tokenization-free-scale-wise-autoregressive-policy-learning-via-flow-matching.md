...
tldr: 本研究针对视觉运动策略学习，提出了一种无需分词器的多尺度自回归模型HiFlow。传统方法依赖离散分词器，导致低维动作产生量化误差且训练繁琐。HiFlow直接在连续动作空间操作，通过时间池化构建多尺度目标，并利用流匹配技术进行由粗到细的细化。该方法支持单阶段端到端训练，在MimicGen、RoboTwin 2.0及真实场景中均表现优异，超越了现有的扩散模型和离散自回归策略。
selection_source: fresh_fetch
motivation: 传统自回归策略使用的离散分词器在处理连续机器人动作时会引入不必要的量化误差，并增加多阶段训练的复杂性。
method: HiFlow通过简单的时间池化生成多尺度连续动作序列，并采用流匹配技术实现从粗略摘要到精细动作的端到端自回归细化。
result: 实验证明HiFlow在多个仿真基准测试和真实世界任务中，性能均显著优于扩散策略和基于分词器的自回归方法。
conclusion: 证明了在连续动作空间进行多尺度自回归建模的有效性，为高效、高精度的机器人策略学习提供了新方案。
---
