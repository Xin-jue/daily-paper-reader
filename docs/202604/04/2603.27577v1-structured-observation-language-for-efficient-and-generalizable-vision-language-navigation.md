...
method: "将RGB-D图像划分为N*N网格并提取各单元格的语义、颜色和深度信息，转化为结构化文本作为PLM的纯语言输入。"
result: 在R2R、RxR基准测试和真实世界部署中，该方法显著降低了模型参数量和对训练数据的依赖。
conclusion: SOL-Nav证明了将视觉观测转化为结构化语言能有效提升导航任务的效率、推理能力和跨环境泛化性。
---
