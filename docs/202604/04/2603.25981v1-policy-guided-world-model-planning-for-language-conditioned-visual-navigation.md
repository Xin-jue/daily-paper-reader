...
tldr: 针对语言引导视觉导航中反应式策略缺乏长程规划能力以及世界模型动作初始化困难的问题，本文提出PiJEPA框架。该方法结合了Octo通用策略与JEPA世界模型，利用策略生成的动作分布作为先验来引导模型预测路径积分（MPPI）规划。通过在潜在空间进行规划，PiJEPA在真实导航任务中显著提升了目标到达率和指令遵循精度。
