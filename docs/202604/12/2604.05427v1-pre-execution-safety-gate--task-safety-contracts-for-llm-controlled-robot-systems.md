...
tags: ["query:q4"]
score: 8.0
evidence: 大语言模型控制的机器人系统与自然语言任务指令
...
## 摘要
大语言模型（LLMs）越来越多地被用于将任务指令转换为机器人可执行代码，然而，该流程缺乏验证门，无法在指令被翻译为机器人代码之前检测出不安全和有缺陷的指令。此外，在缺乏持续约束监控的情况下，即使最初看起来安全的指令在执行过程中也可能产生不安全的状态转换。在本研究中，我们引入了 SafeGate，这是一种神经符号安全架构，旨在防止不安全的自然语言任务指令进入机器人执行阶段。借鉴 ISO 13482 安全标准，SafeGate 从自然语言指令中提取结构化的安全相关属性，并应用确定性决策门来授权或拒绝执行。此外，我们引入了任务安全契约（Task Safety Contracts），将通过安全门的指令分解为不变式、守卫条件和中止条件，以防止执行过程中的不安全状态转换。我们进一步结合了 Z3 SMT 求解，以强制执行源自任务安全契约的约束检查。我们在 230 个基准任务、30 个 AI2-THOR 模拟场景以及真实世界机器人实验中，将 SafeGate 与现有的基于 LLM 的机器人安全框架和基准 LLM 进行了评估。结果表明，SafeGate 在保持对良性任务高接受率的同时，显著降低了对缺陷指令的接受率，证明了执行前安全门对于大语言模型控制的机器人系统的重要性。
