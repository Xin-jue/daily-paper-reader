...
tags: ["query:q8"]
score: 8.0
evidence: 利用世界模型和LLM实现四足机器人的仿真到现实迁移
...
## 摘要
在多样且动态的地形上实现四足机器人运动面临着重大挑战，这主要是由于仿真环境与现实世界条件之间的差异。传统的从仿真到现实（sim-to-real）迁移方法通常依赖于手动特征设计或昂贵的现实世界微调。为了解决这些局限性，本文提出了 DreamTIP 框架，该框架将任务不变属性（Task-Invariant Properties）学习整合到 Dreamer 世界模型架构中，以增强 sim-to-real 迁移能力。在大语言模型的指导下，DreamTIP 识别并利用了任务不变属性（如接触稳定性和地形间隙），这些属性对动态变化具有鲁棒性，并在不同任务间具有很强的迁移性。这些属性作为辅助预测目标被集成到世界模型中，使策略能够学习到对底层动态变化不敏感的表示。此外，本文设计了一种高效的自适应策略，利用混合回放池和正则化约束，在快速校准到现实世界动态的同时，有效缓解了表示崩溃和灾难性遗忘。在包括楼梯、爬坡、倾斜和爬行在内的复杂地形上进行的广泛实验表明，DreamTIP 在仿真和现实环境中均显著优于现有的基准方法。我们的方法在八个不同的仿真迁移任务中实现了平均 28.1% 的性能提升。在现实世界的爬坡任务中，基准方法的成功率仅为 10%，而我们的方法达到了 100% 的成功率。这些结果表明，将任务不变属性纳入 Dreamer 学习，为实现鲁棒且可迁移的机器人运动提供了一种新颖的解决方案。
