...
tags: ["query:q4"]
score: 7.0
evidence: 视觉语言引导的四旋翼敏捷飞行与场景记忆
tldr: QuadAgent是一个无需训练的四旋翼无人机智能体系统，旨在通过视觉语言指令实现敏捷飞行。它采用异步多智能体架构，将高层推理与底层控制解耦，通过前台智能体处理任务，后台智能体进行前瞻性推理。系统利用轻量化拓扑图“印象图”维护场景记忆，并结合避障网络确保安全。实验证明其在复杂指令理解和避障导航方面表现优异，实机飞行速度可达5m/s。
selection_source: fresh_fetch
motivation: 旨在解决传统端到端或串行智能体在视觉语言引导的敏捷飞行中，高层推理与底层实时控制难以高效协同的问题。
method: 采用异步多智能体架构，结合前后台智能体协作、轻量化拓扑地图（印象图）以及视觉避障网络实现解耦控制。
result: 实验显示该系统在处理复杂指令和环境推理方面优于基准方法，并在实机测试中实现了5m/s的室内避障飞行。
conclusion: QuadAgent通过解耦推理与控制，为视觉语言引导的无人机敏捷导航提供了一种高效、安全且无需训练的新型架构。
---
