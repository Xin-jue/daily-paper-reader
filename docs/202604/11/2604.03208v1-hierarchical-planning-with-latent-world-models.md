...
tags: ["query:q13"]
score: 8.0
evidence: 基于潜空间世界模型的分层规划用于具身控制
tldr: 针对学习世界模型在长时程控制中面临的预测误差累积和搜索空间爆炸问题，本文提出了一种多时间尺度潜空间世界模型的分层规划方法。该方法通过在不同时间尺度上学习潜空间模型并进行分层规划，显著降低了推理复杂度。实验证明，该方法在机器人抓取、迷宫导航等任务中表现优异，不仅在零样本控制下实现了极高的成功率，还大幅减少了计算开销。
selection_source: fresh_fetch
motivation: 现有的学习世界模型在处理长时程任务时，受限于预测误差累积和指数级增长的搜索空间。
method: 提出一种模块化的分层规划框架，在多个时间尺度上学习潜空间世界模型，并跨尺度进行分层推理。
result: "在真实机器人拾取任务中实现70%的成功率（对比单层模型的0%），且在仿真环境中计算量减少达4倍。"
conclusion: 分层潜空间规划能有效提升长时程任务的推理能力和计算效率，是增强具身智能泛化能力的有效途径。
---
