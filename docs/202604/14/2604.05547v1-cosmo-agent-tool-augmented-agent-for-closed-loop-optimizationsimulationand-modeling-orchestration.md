...
tags: ["query:q11"]
score: 6.0
evidence: 用于设计-仿真的自动化环境合成
tldr: 针对工业设计中CAD与CAE之间的语义鸿沟，本文提出COSMO-Agent框架。该框架利用强化学习训练大语言模型，使其能够协同调用外部工具完成几何生成、仿真求解及反馈修正的闭环过程。通过设计多约束奖励机制和构建包含25类组件的工业数据集，COSMO-Agent显著提升了模型在约束驱动设计中的可行性与效率，使小型开源模型表现优于大型闭源模型。
selection_source: fresh_fetch
motivation: 工业设计优化受阻于CAD与CAE之间的语义鸿沟，难以根据仿真反馈自动进行有效的几何修正。
method: 提出一种工具增强的强化学习框架，通过多约束奖励机制训练LLM协同外部工具链实现闭环的参数化几何优化。
result: 实验表明该方法显著提升了小型开源模型在约束驱动设计任务中的可行性、效率和稳定性，超越了强力闭源模型。
conclusion: COSMO-Agent成功实现了CAD-CAE闭环流程的自动化编排，为工业级智能设计优化提供了高效且稳健的解决方案。
---

## 摘要
迭代式工业设计-仿真优化受限于 CAD-CAE 语义鸿沟：即在多样且耦合的约束条件下，如何将仿真反馈转化为有效的几何编辑。为了填补这一空白，我们提出了 COSMO-Agent（闭环优化、仿真与建模编排），这是一个工具增强的强化学习（RL）框架，旨在教导大语言模型（LLM）完成闭环 CAD-CAE 流程。具体而言，我们将 CAD 生成、CAE 求解、结果解析和几何修正建模为一个交互式强化学习环境，LLM 在其中学习编排外部工具并修正参数化几何体，直到满足约束条件。为了使学习过程稳定且具备工业可用性，我们设计了一种多约束奖励机制，共同激励可行性、工具链鲁棒性和结构化输出的有效性。此外，我们贡献了一个符合工业标准的动态数据集，涵盖 25 个组件类别，包含可执行的 CAD-CAE 任务，以支持真实的训练和评估。实验表明，COSMO-Agent 训练显著提升了小型开源 LLM 在约束驱动设计方面的表现，在可行性、效率和稳定性方面超过了大型开源模型和强力闭源模型。
