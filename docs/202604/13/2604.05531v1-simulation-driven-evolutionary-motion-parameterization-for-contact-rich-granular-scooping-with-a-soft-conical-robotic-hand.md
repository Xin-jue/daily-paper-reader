...
## 摘要
基于工具的舀取在机器人辅助任务中至关重要，能够实现与不同尺寸、形状和材料状态物体的交互。近期研究表明，柔性、可重构的软体机器人末端执行器可以调整其形状，以在舀取过程中与容器表面保持一致的接触，从而提高相比于刚性工具的效率。这些软体工具可以适应不同的容器尺寸和材料，而无需复杂的传感或控制。然而，软体机器人固有的顺应性和复杂的变形行为引入了显著的控制复杂性，限制了其实际应用。为应对这一挑战，本文开发了一个可变形软体圆锥形机器人手的物理仿真模型，该模型捕捉了其被动重构动力学，并实现了舀取任务的系统化轨迹优化。我们提出了一种新颖的基于物理的仿真方法，能够准确模拟软体工具从平面薄片到自适应圆锥结构的变形行为，并结合演化策略框架，在无需人工参数调整的情况下自动优化舀取轨迹。我们通过仿真和真实机器人实验验证了优化后的轨迹。结果表明该方法具有很强的泛化能力，并成功解决了以往方法无法处理的一系列具有挑战性的任务。实验视频可在以下网址在线获取：https://sites.google.com/view/scoopsh
