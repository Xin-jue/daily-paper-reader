...
tags: ["query:q12"]
score: 8.0
evidence: 用于物理 AI 和机器人的高效 VLM 架构
tldr: Fast-dVLM 针对自回归视觉语言模型（VLM）推理速度慢的问题，提出了一种基于块扩散（block-diffusion）的高效架构。通过将预训练的 AR VLM 直接转换为扩散模型，并引入块大小退火、因果上下文注意力和视觉高效拼接等技术，实现了与 KV 缓存兼容的并行解码。在保持生成质量的同时，Fast-dVLM 在 11 个基准测试中表现优异，并实现了超过 6 倍的推理加速，显著提升了边缘设备上的部署效率。
selection_source: fresh_fetch
motivation: 自回归解码逐个生成 token 导致推理吞吐量受限，尤其在机器人和自动驾驶等边缘计算场景下硬件并行度利用不足。
method: 采用直接将预训练 AR VLM 转换为块扩散模型的一阶段策略，并结合块大小退火、因果上下文注意力和自动截断掩码等优化手段。
result: 在 11 个多模态基准测试中达到了与自回归模型相当的生成质量，并在集成 SGLang 和 FP8 量化后实现了超过 6 倍的端到端推理加速。
conclusion: 直接从 AR VLM 转换而来的块扩散模型是实现高效、高质量多模态推理的有效途径，特别适用于对实时性要求高的物理 AI 场景。
---

## 摘要
视觉语言模型（VLM）主要依赖自回归解码，这种方式一次生成一个 token，从根本上限制了推理吞吐量。这种限制在机器人和自动驾驶等物理 AI 场景中尤为突出，在这些场景中，VLM 以 batch size 为 1 的形式部署在边缘设备上，使得自回归（AR）解码受限于内存带宽，导致硬件并行性未得到充分利用。虽然分块离散扩散在并行文本生成方面展现出潜力，但由于需要同时处理连续的视觉表示和离散的文本 token，并保持预训练的多模态能力，将其扩展到 VLM 仍具挑战性。我们提出了 Fast-dVLM，这是一种基于块扩散的 VLM，支持与 KV 缓存兼容的并行解码和投机块解码，以实现推理加速。我们系统地比较了两种从 AR 到扩散的转换策略：一种是两阶段方法，即在多模态训练之前先通过纯文本扩散微调来适配 LLM 主干网络；另一种是直接方法，即在一个阶段内完成完整 AR VLM 的转换。在相当的训练预算下，直接转换通过利用已经实现多模态对齐的 VLM，证明了其效率显著更高；因此，我们将其作为推荐方案。我们引入了一系列多模态扩散适配技术，包括块大小退火、因果上下文注意力、自动截断掩码以及视觉高效拼接，这些技术共同实现了 VLM 环境下的有效块扩散。在 11 个多模态基准测试上的广泛实验表明，Fast-dVLM 在生成质量上与对应的自回归模型相当。通过 SGLang 集成和 FP8 量化，Fast-dVLM 相比 AR 基准模型实现了超过 6 倍的端到端推理加速。
