...
score: 7.0
evidence: 具身智能体与机器人及环境交互的运行时治理
tldr: 随着具身智能体从推理转向主动执行，如何确保其行为在运行时受控成为核心挑战。本文提出了一种策略约束执行框架，将智能体的认知与执行监管分离，通过独立的运行时治理层实现策略检查、监控和故障恢复。实验表明，该方法能有效拦截违规行为并显著提升系统在复杂环境下的合规性与恢复能力，为具身智能系统的安全设计提供了新思路。
selection_source: fresh_fetch
motivation: 现有的具身智能体安全逻辑通常嵌入在智能体内部，导致执行控制难以标准化、审计和灵活调整。
method: 提出一种将智能体认知与执行监管分离的运行时治理框架，通过独立的治理层进行策略校验、能力准入和执行监控。
result: "在1000次模拟实验中，该框架实现了96.2%的违规拦截率，并将运行时偏移下的不安全持续行为从100%降至22.2%。"
conclusion: 将运行时治理视为系统设计的核心问题，是实现受控且可靠的具身智能体执行的关键原则。
---
