...
tldr: ROBOGATE是一个旨在提高机器人策略部署安全性的风险管理框架。它结合物理仿真与两阶段自适应采样策略（LHS粗采样与边界聚焦采样），在8维参数空间中高效识别失效边界。通过在Franka和UR5e机器人上进行3万次实验，该框架不仅能精确建模失效风险，还揭示了基础模型（如Octo-Small）在工业场景中的脆弱性，为机器人策略的预部署验证提供了有力工具。
selection_source: carryover_cache
motivation: 工业机器人策略部署需要严格验证，但高维参数空间的穷举测试成本过高且难以实现。
method: 提出两阶段采样策略，先通过拉丁超立方采样建立粗略景观，再针对成功率过渡区进行边界聚焦采样以精确映射失效区域。
result: 实验表明该框架能显著提升风险预测准确度（AUC达0.780），并发现VLA模型在对抗性场景下表现远逊于脚本化基准。
conclusion: ROBOGATE通过高效的失效边界发现机制，为评估和提升机器人策略（尤其是基础模型）在复杂工业环境中的安全性提供了重要参考。
---
