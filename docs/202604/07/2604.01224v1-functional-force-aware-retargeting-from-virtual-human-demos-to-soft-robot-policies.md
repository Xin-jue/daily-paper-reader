...
tldr: 本研究提出SoftAct框架，旨在通过显式推理接触力，将人类操纵技能迁移至软机器人手。该系统利用VR捕获包含运动学、接触面及力信息的演示，并采用两阶段力感知重定向算法：首先在人机手指间分配力平衡映射，随后结合末端位姿跟踪与测地线加权接触细化进行实时调整。实验表明，该方法在处理极端形态差异和非线性顺应性方面表现优异，显著提升了软机器人在复杂接触任务中的模仿精度与成功率。
selection_source: fresh_fetch
motivation: 由于软机器人手存在非线性顺应性和显著的形态差异，传统的运动学模仿难以有效转移人类的操纵技能。
method: 提出一种两阶段力感知重定向算法，通过VR捕获的接触几何与力分布信息，实现从人类演示到机器人策略的功能性映射。
result: "实验显示SoftAct将指尖轨迹跟踪误差降低了55%，并在零样本真实世界部署中取得了比基准方法更高的成功率。"
conclusion: 显式建模接触力和几何分布是实现软机器人技能高效迁移的关键，其效果远优于单纯的运动学模仿。
---

## 摘要
我们介绍了 SoftAct，这是一个通过显式推理接触力来教导软体机器人手执行类人操纵技能的框架。利用沉浸式虚拟现实，我们的系统捕获了丰富的人类演示，包括手部运动学、物体运动、密集接触区域以及详细的接触力信息。与重定向人类关节轨迹的传统方法不同，SoftAct 采用了一种两阶段的力感知重定向算法。第一阶段将演示的接触力归因于单个人类手指，并按比例分配机器人手指，从而在人类手和机器人手之间建立力平衡映射。第二阶段通过将基准末端执行器位姿跟踪与测地线加权接触细化相结合来执行在线重定向，利用接触几何和力的大小实时调整机器人指尖目标。这种公式化方法使软体机器人手能够重现人类演示的功能意图，同时自然地适应极端的具身失配和非线性顺应性。我们使用定制的非拟人化气动软体机器人手，在一系列接触丰富的操纵任务上评估了 SoftAct。与运动学和基于学习的基准相比，SoftAct 的控制器将指尖轨迹跟踪的均方根误差（RMSE）降低了高达 55%，并将跟踪方差降低了高达 69%。在策略层面，SoftAct 在零样本现实世界部署和仿真中均实现了持续更高的成功率。这些结果表明，显式建模接触几何和力分布对于向软体机器人手进行有效的技能迁移至关重要，且无法仅通过运动学模仿来恢复。项目视频和更多细节可在 https://soft-act.github.io/ 获取。
