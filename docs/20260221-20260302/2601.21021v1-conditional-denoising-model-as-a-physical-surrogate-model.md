...
conclusion: 去噪训练目标可作为强大的隐式正则化器，使模型在不直接学习控制方程的情况下实现比显式物理损失更强的物理一致性。
---

## 摘要
复杂物理系统的代理建模通常面临数据拟合精度与物理一致性之间的权衡。物理一致性方法通常将物理定律视为损失函数中的软约束，这种策略往往无法保证严格遵守控制方程，或者依赖于无法从本质上学习底层解几何结构的后处理修正。为了解决这些局限性，我们引入了条件去噪模型（CDM），这是一种旨在学习物理流形本身几何结构的生成模型。通过训练网络从噪声状态恢复出干净状态，该模型学习了一个持续指向有效解子空间的向量场。我们引入了一种与时间无关的公式，将推理转化为确定性的不动点迭代，从而有效地将噪声近似投影到平衡流形上。在低温等离子体物理与化学基准测试中，CDM 实现了比物理一致性基准模型更高的参数效率和数据效率。至关重要的是，我们证明了去噪目标起到了强大的隐式正则化作用：尽管在训练过程中从未接触过控制方程，该模型比使用显式物理损失训练的基准模型更严格地遵守物理约束。
