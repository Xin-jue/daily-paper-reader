...
tags: ["query:sr"]
score: 7.0
evidence: KAN作为MLP的可解释替代方案
tldr: 本研究评估了 Kolmogorov-Arnold 网络 (KAN) 在 33 种材料属性预测中的表现。实验表明，调优后的 KAN 精度与随机森林相当，且在反应堆压力容器脆化预测中，仅需极少参数即可生成与专家物理模型性能一致的闭式表达式。这证明了 KAN 在材料科学中具有极高的参数效率和卓越的可解释性，是极具潜力的回归建模工具。
selection_source: fresh_fetch
motivation: 旨在评估 KAN 这一新型架构在材料科学回归任务中相较于传统机器学习模型的参数效率和可解释性优势。
method: 在 33 种材料属性数据集上应用 KAN，并将其预测性能与随机森林以及基于领域专家知识的手动模型进行系统对比。
result: 调优后的 KAN 性能与随机森林相当，且能以极少参数生成与专家物理模型精度匹配的闭式数学表达式。
conclusion: KAN 凭借其高参数效率和自动发现物理规律的能力，成为材料科学属性预测中极具潜力的回归模型。
---
