...
tldr: 针对多模态大模型（MLLM）在长程空间推理和心理导航能力的缺失，本文借鉴生物智能提出了Video2Mental评测基准，用于评估模型从第一视角视频构建认知地图并进行路径规划的能力。同时，研究者开发了NavMind模型，通过将显式认知地图作为可学习的中间表示，并采用分层渐进式微调，显著提升了模型在复杂环境下的导航规划性能，填补了AI与生物智能在空间认知上的差距。
selection_source: fresh_fetch
motivation: 现有具身智能模型多依赖即时观察的反应式规划，缺乏生物智能中构建空间表征并进行心理模拟的“心理导航”能力。
method: 提出了Video2Mental基准，并开发了NavMind模型，利用显式认知地图作为中间推理层，结合难度分层的渐进式监督微调。
result: 实验表明前沿MLLM在零样本空间表征上表现不佳，而NavMind在心理导航任务中显著优于现有的商业和空间大模型。
conclusion: 将显式认知地图内化为中间表示是提升大模型长程空间推理和心理导航能力的关键路径。
---

## 摘要
尽管多模态大语言模型（MLLMs）在具身智能体中得到了广泛应用，但它们的能力仍主要局限于基于即时观察的反应式规划，在跨越广泛时空尺度的空间推理方面始终表现不佳。认知科学表明，生物智能（BI）依赖于“心理导航”：即根据经验策略性地构建空间表征，并在行动前对路径进行心理模拟。为了弥合人工智能与生物智能之间的差距，我们推出了 Video2Mental，这是一个用于评估 MLLMs 心理导航能力的开创性基准。该任务要求从长篇第一人称视角视频中构建分层认知地图，并逐步生成基于地标的路径规划，其规划准确性通过基于模拟器的物理交互进行验证。我们的基准测试结果表明，心理导航能力并不会从标准预训练中自然涌现。前沿 MLLMs 在零样本结构化空间表征方面面临巨大挑战，且其规划准确性在长距离范围内急剧下降。为了克服这一问题，我们提出了 NavMind，这是一种推理模型，它使用显式的细粒度认知地图作为可学习的中间表征，从而将心理导航内化。通过难度分层的渐进式监督微调范式，NavMind 有效地弥合了原始感知与结构化规划之间的差距。实验表明，NavMind 实现了卓越的心理导航能力，显著优于前沿的商业和空间 MLLMs。
