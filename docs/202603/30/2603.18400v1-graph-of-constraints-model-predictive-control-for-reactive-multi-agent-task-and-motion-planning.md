...
title_zh: 面向反应式多智能体任务与运动规划的约束图模型预测控制
...
method: 提出了一种基于约束图的模型预测控制（GoC-MPC）框架，利用3D关键点定义几何约束，实现动态任务协调与恢复。
result: 实验表明，该方法在多智能体操纵任务中比现有基准具有更高的成功率、更快的计算速度和更短的路径。
conclusion: GoC-MPC为现实世界扰动下的多智能体协作操纵提供了一种高效、鲁棒且无需预训练的解决方案。
---

## 摘要
相互依赖的几何约束序列是许多多智能体任务与运动规划（TAMP）问题的核心。然而，现有的处理此类约束序列的方法在处理偏序任务和动态智能体分配方面存在困难。它们通常假设分配是静态的，且当扰动改变任务分配时无法进行调整。为了克服这些局限性，我们引入了约束图模型预测控制（GoC-MPC），这是一种与 MPC 集成的通用约束序列框架。GoC-MPC 自然地支持偏序任务、动态智能体协作以及扰动恢复。通过在跟踪的 3D 关键点上定义约束，我们的方法能够鲁棒地解决多种多智能体操作任务——仅根据视觉观测结果即可在线协调智能体并进行调整，而不依赖于训练数据或环境模型。实验表明，与最近的基准方法相比，GoC-MPC 实现了更高的成功率、显著更快的 TAMP 计算速度以及更短的总路径，证明了它是应对现实世界扰动下多智能体操作的一种高效且鲁棒的解决方案。我们的补充视频和代码可在 https://sites.google.com/view/goc-mpc/home 找到。
