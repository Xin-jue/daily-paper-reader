...
tldr: 针对大语言模型在任务规划中容易产生幻觉且难以识别结构性错误的问题，本文提出了GNNVerifier。该方法将规划表示为有向图，利用图神经网络（GNN）进行结构化评估和错误定位。通过自动生成的扰动数据训练，GNNVerifier能指导LLM进行局部修正。实验证明，该方法在多种数据集和模型上显著提升了规划质量。
selection_source: fresh_fetch
motivation: 现有的基于LLM的验证器容易被虚假叙述误导，且难以检测步骤间的类型不匹配或依赖断裂等结构性错误。
method: 将规划转换为包含子任务节点和依赖边属性的有向图，利用GNN输出全局可信度及局部风险评分，引导LLM进行局部编辑。
result: 在多种数据集和LLM骨干网络上的实验表明，GNNVerifier能有效识别错误并显著提升任务规划的成功率。
conclusion: 通过引入图结构评估机制，GNNVerifier为LLM任务规划提供了一种比传统自反思方法更精准、更具结构感知力的验证手段。
---
