...
tldr: 针对大语言模型在机器人规划中面临的歧义性挑战，本文提出SG-CoT框架。该框架通过构建环境的场景图表示，并结合检索函数让LLM迭代查询相关信息，从而将推理过程锚定在实际观测中。SG-CoT能识别歧义并主动提问，显著提升了单智能体和多智能体环境下的规划成功率。
selection_source: fresh_fetch
motivation: 解决大语言模型作为机器人规划器时，因环境信息模糊或指令不明确而导致的规划失败问题。
method: 采用两阶段框架，先构建包含物体属性和关系的场景图，再利用思维链结合检索函数让LLM查询场景图以消除歧义。
result: "实验显示该方法在提问准确率上提升至少10%，在单智能体和多智能体任务的成功率上分别提升了4%和15%。"
conclusion: SG-CoT通过场景图锚定推理过程，有效增强了机器人规划器在复杂模糊环境下的可靠性和泛化能力。
---
