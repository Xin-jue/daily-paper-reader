...
tldr: OnlinePG是一个基于3D高斯泼溅（3DGS）的在线全景建图系统，旨在解决现有方法在具身智能应用中缺乏实时性或实例级理解的问题。该系统采用局部到全局的滑动窗口范式，通过3D片段聚类图实现局部一致性，并利用空间属性网格和双向二分匹配技术更新全局地图，最终结合视觉语言模型（VLM）特征实现高效的在线开放词汇场景理解。
selection_source: fresh_fetch
motivation: 现有的开放词汇场景理解方法多为离线模式或缺乏实例级理解，难以满足机器人实时交互的需求。
method: 提出一种基于3DGS的局部到全局建图框架，结合3D片段聚类图进行局部融合，并通过空间属性网格和双向二分匹配实现全局地图更新。
