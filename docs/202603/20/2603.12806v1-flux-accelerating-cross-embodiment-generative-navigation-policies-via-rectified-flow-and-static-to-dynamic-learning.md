...
tags: ["query:q13"]
score: 6.0
evidence: 生成式导航策略与动态人群仿真
tldr: 本研究针对自主导航评估碎片化的问题，推出了包含物理仿真人群的动态导航基准DynBench，并提出了首个基于流的统一导航策略FLUX。FLUX通过修正流技术将迭代去噪简化为直线轨迹，显著提升了推理效率。该方法采用从静态到动态的课程学习方案，先建立几何先验再通过强化学习优化社交避障能力。实验表明，FLUX在多项任务中达到SOTA，并成功在轮式、四足及人形机器人上实现零样本跨具身迁移。
selection_source: fresh_fetch
motivation: 现有的自主导航评估协议碎片化，且缺乏能同时处理静态目标到达和动态社交避障的统一高效生成式策略。
method: 提出FLUX策略，利用修正流线性化概率流以提高推理速度，并结合从静态几何先验到动态强化学习的课程学习方法。
result: "FLUX在六项导航任务中均取得最优性能，推理效率比扩散模型提升29%，并实现了在多种机器人平台上的零样本虚实迁移。"
conclusion: FLUX证明了基于流的生成式模型在跨具身、多任务导航中的高效性与鲁棒性，为通用导航策略提供了新的技术路径。
---
