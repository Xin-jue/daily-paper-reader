...
tldr: 针对具身多智能体系统中局部视角导致的感知局限，本文提出了Ego-to-World (E2W) 基准测试，用于评估视觉语言模型融合异构视角的能力。同时提出CoRL框架，通过思维链微调和强化学习（结合跨视角空间奖励CVSR）提升空间推理性能。实验证明CoRL在推理和感知任务上均优于现有模型，并成功应用于真实世界的多机器人协作。
selection_source: fresh_fetch
motivation: 具身智能体通常受限于局部视角、遮挡和歧义，难以从多个异构的自我中心视角构建全局一致的世界理解。
method: 提出CoRL框架，结合思维链监督微调与强化学习，并利用跨视角空间奖励（CVSR）为推理步骤提供密集的视觉对齐反馈。
result: CoRL在E2W基准测试的推理与感知指标上显著超越了主流模型，并成功实现了真实世界中多摄像机协作的机器人抓取任务。
conclusion: E2W基准和CoRL框架为从分布式局部观测中学习全局场景理解奠定了基础，有效推动了协作式具身智能的发展。
---

## 摘要
从分布式的局部视角理解世界是具身多智能体系统面临的一项根本挑战。每个智能体通过自我中心视角感知环境，而这种视角通常受到遮挡和歧义的限制。为了研究这一问题，我们引入了 Ego-to-World (E2W) 基准测试，用于评估视觉语言模型在三项任务中融合异构视角的能力：(i) 全局计数，(ii) 关系位置推理，以及 (iii) 需要预测特定视角图像坐标的面向动作的抓取。针对这一设定，我们提出了 CoRL，这是一个结合了思维链（Chain-of-Thought）监督微调与使用群体相对策略优化（Group-Relative Policy Optimization）的强化学习的两阶段框架。其核心组件跨视图空间奖励（Cross-View Spatial Reward, CVSR）通过将推理步骤与视觉证据联系起来，提供密集的任务对齐反馈，确保连贯的跨视图实体解析，并引导模型做出正确的最终预测。在 E2W 上的实验表明，CoRL 在推理和感知定位指标上均持续超越了强大的闭源和开源基线模型，消融实验进一步证实了 CVSR 各个组件的必要性。此外，CoRL 能够泛化到外部空间推理基准测试，并能利用校准后的多摄像头装置实现有效的真实世界多机器人操控，展示了跨视图定位和成功的抓取并放置执行。总之，E2W 和 CoRL 为从分布式的自我中心观测中学习以世界为中心的场景理解提供了原则性基础，推动了协作具身智能的发展。
