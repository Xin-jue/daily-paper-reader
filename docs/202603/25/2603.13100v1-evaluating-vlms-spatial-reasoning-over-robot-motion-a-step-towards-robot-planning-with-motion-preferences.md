...
tldr: 本研究评估了视觉语言模型（VLM）在机器人运动空间推理方面的能力，旨在探索其在满足用户运动偏好（如物体距离、路径风格）方面的潜力。通过对四种主流VLM进行四种查询方法的测试，发现Qwen2.5-VL在零样本和微调后均表现出色，优于GPT-4o。研究分析了准确率与计算成本的权衡，证明了将VLM集成到机器人运动规划流程中的可行性。
selection_source: carryover_cache
motivation: 旨在探究视觉语言模型是否具备理解和执行机器人运动中空间约束及用户偏好的空间推理能力。
method: 采用四种不同的查询方法，对四种最先进的VLM在物体接近度和路径风格两种运动偏好任务上进行了评估。
result: "Qwen2.5-VL在零样本测试中达到71.4%的准确率，微调后提升至75%，表现优于GPT-4o。"
conclusion: 研究结果表明VLM在机器人运动规划集成方面具有潜力，并揭示了模型准确率与计算成本之间的权衡关系。
---

## 摘要
理解用户指令和周围环境中的物体空间关系，对于智能机器人系统协助人类完成各种任务至关重要。视觉语言模型（VLM）的自然语言和空间推理能力，具有增强机器人规划器在面对新任务、新物体和运动规范时泛化能力的潜力。虽然基础模型已被应用于任务规划，但目前尚不清楚它们在多大程度上具备执行用户运动偏好或约束（如与物体的期望距离、拓扑属性或运动风格偏好）所需的空间推理能力。在本文中，我们使用四种不同的查询方法，评估了四种最先进的 VLM 在机器人运动上的空间推理能力。结果表明，在使用性能最高的查询方法时，Qwen2.5-VL 在零样本（zero-shot）下达到了 71.4% 的准确率，而较小模型在微调后达到了 75%，GPT-4o 的表现则相对较低。我们评估了两种类型的运动偏好（物体接近度和路径风格），并分析了准确率与以 token 数量衡量的计算成本之间的权衡。这项工作展示了将 VLM 集成到机器人运动规划流水线中的潜力。
