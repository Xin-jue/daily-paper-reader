...
score: 7.0
evidence: 用于机器人操作和导航的3D场景理解
tldr: OGScene3D 针对机器人增量探索环境的需求，提出了一种基于 3D 高斯泼溅的增量式开放词汇场景图构建系统。该系统通过置信度感知的语义表示和层次化优化策略，实现了全局一致的语义映射。结合长期全局优化和渐进式图构建，OGScene3D 能够动态更新场景节点及关系，在开放词汇场景理解任务中表现优异，有效支持机器人导航与操作等下游任务。
selection_source: fresh_fetch
motivation: 现有开放词汇场景理解方法通常依赖预先构建的完整地图，限制了其在机器人增量探索未知环境时的应用。
method: 采用置信度感知的 3D 高斯语义表示，结合层次化语义优化、长期全局优化及渐进式图构建技术，实现动态语义映射与场景图生成。
result: 在主流数据集和真实场景中的实验证明，该方法在开放词汇语义映射和场景图构建方面具有显著的准确性和一致性。
conclusion: OGScene3D 为机器人在动态探索过程中实时构建和理解复杂 3D 场景语义关系提供了一种高效且鲁棒的解决方案。
---
