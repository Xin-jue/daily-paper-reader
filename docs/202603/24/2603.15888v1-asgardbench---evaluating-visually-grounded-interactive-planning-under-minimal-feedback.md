...
## 摘要
通过 AsgardBench，我们旨在评估视觉落地的、高层动作序列生成和交互式规划，特别关注执行过程中基于视觉观察而非导航或底层操作的规划调整。在具身智能（Embodied AI）基准测试的版图中，AsgardBench 针对的是交互式规划这一能力类别，这比离线高层规划更复杂，因为它要求智能体根据环境反馈修正规划，但又与底层执行有所区别。与以往将推理与导航混淆，或提供丰富的纠错反馈以替代感知的具身智能基准不同，AsgardBench 将智能体的输入限制为图像、动作历史和轻量级的成功/失败信号，从而在没有底层控制噪声的受控模拟器中隔离出交互式规划。该基准包含跨越 12 种任务类型的 108 个任务实例，每个实例都通过物体状态、放置位置和场景配置进行了系统性的变化。这些受控的变化创造了条件分支，使得同一条指令可能根据智能体观察到的情况需要不同的动作序列，强调了执行过程中的条件分支和规划修复。我们对领先的视觉语言模型的评估表明，在没有视觉输入的情况下性能会急剧下降，揭示了视觉落地和状态跟踪方面的弱点，这些弱点最终损害了交互式规划。我们的基准测试聚焦于一个更具体的问题：当情况不符合预期时，模型是否真的能利用其所见来调整规划？
