...
tldr: AutoMoT是一个统一的视觉-语言-动作（VLA）模型，旨在解决端到端自动驾驶中推理与动作空间不匹配及推理延迟高的问题。该模型采用异步混合Transformer（MoT）架构，通过共享注意力机制保留预训练VLM的推理能力，并支持不同频率的快慢异步推理。实验证明其在多项基准测试中表现优异，并揭示了预训练模型在场景理解与动作规划任务中的功能边界。
selection_source: fresh_fetch
motivation: 现有端到端自动驾驶系统在集成视觉语言模型时，面临推理与动作分布不一致、预训练能力利用不足以及推理延迟过高的问题。
method: 提出AutoMoT框架，利用具有联合注意力共享的混合Transformer架构，通过异步执行机制实现高效的快慢任务推理。
result: 在多个开源和闭环基准测试中，AutoMoT达到了与当前最先进方法相当的竞争性能。
