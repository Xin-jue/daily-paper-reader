...
tldr: 本研究针对具身智能中的单目3D语义占据预测任务，提出了SGR-OCC框架。针对单目估计的深度歧义和时序融合的“冷启动”不稳定性，该框架通过软门控特征提升器建模深度不确定性，并利用动态射线约束锚点细化模块实现高效的几何校正。结合两阶段渐进式训练策略，SGR-OCC在ScanNet等基准测试上显著提升了预测精度和边界清晰度，达到了SOTA水平。
selection_source: fresh_fetch
motivation: 旨在解决单目3D占据预测中由于深度歧义导致的特征溢出，以及时序融合初期不稳定性对空间先验的干扰问题。
method: 提出了包含软门控特征提升器、动态射线约束锚点细化及两阶段渐进式训练策略的统一框架，实现从空间先验到时序一致性的平滑演进。
result: "在EmbodiedOcc-ScanNet基准上，该方法在局部预测和具身预测任务中均刷新了SOTA记录，语义mIoU较前最优方法提升了3.69%。"
conclusion: SGR-OCC通过有效继承和演化单目空间先验，显著增强了具身智能体在复杂室内环境中的结构完整性和边界感知能力。
---
