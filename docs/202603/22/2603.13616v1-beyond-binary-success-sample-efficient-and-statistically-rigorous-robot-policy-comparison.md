...
## 摘要
通用机器人操纵策略的能力日益增强，但在评估时受限于少量的硬件部署。现实世界测试中这种强烈的资源限制，要求采用更具信息量的性能衡量标准以及可靠且高效的评估程序，以便妥善评估模型能力并衡量该领域的基准进展。本研究提出了一种新型机器人策略比较框架，该框架具有样本高效性、统计严谨性，并适用于实践中使用的广泛评估指标。基于安全且随时有效推理（SAVI），我们的测试程序是序列化的，允许评估者在积累了足够的统计证据以达到预设置信水平的决策时提前停止。与以往针对二元成功开发的工作不同，我们的统一方法涵盖了广泛的信息化指标：从离散的部分得分任务进度到回合奖励或轨迹平滑度的连续度量，跨越了参数化和非参数化比较问题。通过在模拟和真实世界评估数据上的广泛验证，我们证明了与标准批处理方法相比，评估负担减少了高达 70%，与专为二元结果设计的先进序列程序相比，评估负担减少了高达 50%，且没有损失统计严谨性。值得注意的是，我们的实证结果表明，与二元成功指标相比，使用细粒度的任务进度可以更快地对竞争策略进行区分。
