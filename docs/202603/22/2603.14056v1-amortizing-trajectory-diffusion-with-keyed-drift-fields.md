...
## 摘要
基于扩散的轨迹规划器可以为离线强化学习合成丰富的多模态动作序列，但其迭代去噪过程会产生巨大的推理时间开销，使得在计算预算有限的情况下闭环规划变得缓慢。我们研究了如何在保持采样多样化候选规划以及在退避水平控制循环中以当前状态为条件的能力的同时，通过单步推理实现类似扩散的轨迹规划行为。我们的关键观察是，当用于将生成的轨迹与数据集对齐的相似性度量被不受约束的未来维度所主导时，条件轨迹生成在朴素的分布匹配目标下会失效。在实践中，这会导致轨迹向平均轨迹靠拢，使动作多样性崩溃，并产生近乎静态的行为。我们的核心见解是，条件生成式规划需要一种感知条件的邻域概念：轨迹更新应使用反映条件的紧凑键空间中的距离来计算，同时仍在完整轨迹空间中应用更新。基于此，我们引入了键控漂移策略（Keyed Drifting Policies, KDP），这是一种单步轨迹生成器，使用漂移场目标进行训练。该目标将生成的轨迹吸引向条件匹配的数据集窗口，并使其远离附近的生成样本，同时利用停止梯度的漂移目标将迭代细化过程摊销到训练中。在推理时，所得策略通过单次前向传递即可生成完整的轨迹窗口。在标准强化学习基准测试和实时硬件部署中，KDP 通过单步推理实现了强大的性能，且规划延迟显著低于扩散采样。
