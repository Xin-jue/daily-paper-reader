...
tldr: RoboRouter是一个无需训练的机器人操作策略路由框架。针对现有VLA、VA等策略在特定任务表现优异但泛化性受限的问题，该框架通过维护异构策略池，利用语义表示和历史经验，为新任务智能选择最优策略。实验证明，它在模拟和真实场景中均显著提升了任务成功率，为构建高效、可扩展的机器人系统提供了新路径。
selection_source: carryover_cache
motivation: 现有的机器人操作策略在特定分布内表现良好，但单一模型难以兼顾所有任务的泛化性与成功率。
method: 提出一种无需训练的路由机制，通过构建任务语义表示并结合历史执行反馈，动态地从异构策略池中检索并选择最佳策略。
result: "在模拟环境和真实世界中，RoboRouter分别将平均成功率提升了3%和13%以上，且保持了较高的执行效率。"
conclusion: 通过对现成异构策略进行智能路由，可以在不增加训练成本的情况下，有效提升机器人系统的综合操作能力和可扩展性。
---
