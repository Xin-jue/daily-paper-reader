...
title_zh: GNNVerifier：面向大语言模型任务规划的基于图的验证器
...
tldr: 针对大语言模型在任务规划中容易产生幻觉且难以识别结构性错误的问题，本文提出了GNNVerifier。该方法将规划表示为包含子任务和依赖关系的有向图，利用图神经网络（GNN）进行结构化评估和错误定位。通过自动生成的扰动数据训练，GNNVerifier能有效指导LLM进行局部修正，显著提升了复杂任务规划的质量和可靠性。
selection_source: fresh_fetch
motivation: 现有的基于LLM的验证器容易被合理的叙述误导，且难以检测任务步骤间复杂的结构性错误和依赖缺失。
method: 将规划转换为属性图，利用GNN生成全局可信度评分和局部风险评分，并基于反馈引导LLM进行局部编辑修正。
result: 在多种数据集和LLM骨干网络上的实验证明，GNNVerifier在识别和纠正规划错误方面均取得了显著的性能提升。
conclusion: 通过引入图结构表征和GNN评估机制，该研究为增强自主智能体的任务规划能力提供了一种有效且精准的验证框架。
---

## 摘要
大语言模型（LLMs）促进了自主智能体的发展。作为此类智能体的核心组件，任务规划旨在将复杂的自然语言请求分解为具体的、可解决的子任务。由于 LLM 生成的规划往往容易产生幻觉，且对长上下文提示敏感，近期的研究引入了规划验证器来识别和纠正潜在缺陷。然而，大多数现有方法仍依赖 LLM 作为验证器，通过额外的提示词进行规划审查或自我反思。基于 LLM 的验证器可能会被看似合理的叙述所误导，且难以检测由跨步骤结构关系引起的故障，如类型不匹配、中间体缺失或依赖关系断裂。为了解决这些局限性，我们提出了一种用于 LLM 任务规划的基于图的验证器。具体而言，所提方法包含四个主要组件：首先，我们将规划表示为具有丰富属性的有向图，其中节点表示子任务，边编码执行顺序和依赖约束。其次，利用图神经网络（GNN）进行结构评估和诊断，生成用于规划采纳的图级合理性评分，以及用于定位错误区域的节点/边级风险评分。第三，我们基于地面真值（ground truth）规划图构建可控扰动，并自动生成带有细粒度标注的训练数据。最后，在 GNN 验证器反馈的指导下，当图级评分不足时，我们使 LLM 能够进行局部编辑（如工具替换或插入）以纠正规划。在多种数据集、骨干 LLM 和规划器上进行的广泛实验表明，我们的 GNNVerifier 在提升规划质量方面取得了显著成效。我们的数据和代码已在 https://github.com/BUPT-GAMMA/GNNVerifier 开源。
