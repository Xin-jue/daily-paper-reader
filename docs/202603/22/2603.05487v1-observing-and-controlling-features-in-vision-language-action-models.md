...
tldr: 本研究针对视觉-语言-动作模型（VLA）内部表示复杂、难以解释的问题，提出了特征可观测性与可控制性的新框架。通过线性分类器识别模型内部的线性编码特征，并结合最优控制理论实施轻量级线性干预，实现了对机器人行为的精准引导。在 π0.5 和 OpenVLA 上的实验证明，该方法无需微调即可在保持闭环性能的同时，实现模型输出与用户偏好的实时对齐，为 VLA 的可解释性和在线适配提供了新路径。
selection_source: carryover_cache
motivation: 旨在解决 VLA 模型因多模态输入和混合架构导致的内部表示难以解释以及行为难以精准控制的挑战。
method: 引入特征可观测性和可控制性概念，利用线性分类器提取特征并采用基于最优控制的线性干预技术来重塑内部表示。
result: 在多种 VLA 架构的仿真实验中，该方法成功实现了对机器人行为的可靠引导，且无需对模型进行繁琐的微调。
conclusion: 研究证明 VLA 拥有可解释的内部结构，通过轻量级干预即可实现实时的任务对齐和行为适配。
---
