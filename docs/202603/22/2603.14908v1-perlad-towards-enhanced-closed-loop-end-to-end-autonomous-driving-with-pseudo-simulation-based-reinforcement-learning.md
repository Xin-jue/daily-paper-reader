...
## 摘要
基于模仿学习（IL）的端到端自动驾驶策略在闭环执行中往往表现不佳，这是由于不足的开环训练目标与实际驾驶需求之间存在不匹配。虽然强化学习（RL）通过奖励信号直接优化驾驶目标提供了一种解决方案，但基于渲染的训练环境引入了渲染差距，且由于高昂的计算成本而效率低下。为了克服这些挑战，我们提出了一种用于闭环端到端自动驾驶的新型基于伪仿真的强化学习方法 PerlAD。基于离线数据集，PerlAD 构建了一个在向量空间中运行的伪仿真环境，实现了高效、无需渲染的试错训练。为了弥合静态数据集与动态闭环环境之间的差距，PerlAD 引入了一个预测世界模型，该模型根据主车的规划生成反应式智能体轨迹。此外，为了促进高效规划，PerlAD 采用了一个分层解耦规划器，结合了用于横向路径生成的模仿学习和用于纵向速度优化的强化学习。综合实验结果表明，PerlAD 在 Bench2Drive 基准测试中达到了最先进的性能，在无需昂贵的在线交互的情况下，驾驶分数比之前的端到端强化学习方法高出 10.29%。在 DOS 基准测试上的额外评估进一步证实了其在处理安全至关重要的遮挡场景中的可靠性。
