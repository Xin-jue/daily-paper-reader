...
tags: ["query:q9"]
score: 7.0
evidence: 具身智能中动态场景的语言对齐重建
tldr: 本研究提出SLARM，一种统一动态场景重建、语义理解和实时流式推理的前馈模型。它通过高阶运动建模捕捉复杂非均匀运动，无需光流监督，并蒸馏LSeg语义特征实现语言对齐。采用窗口因果注意力机制，SLARM支持低延迟流式处理且内存成本稳定。该模型在动态估计、渲染质量和场景解析方面均达到SOTA水平，显著提升了运动精度、重建质量和分割性能。
selection_source: fresh_fetch
motivation: 旨在解决动态场景重建中复杂运动捕捉困难、缺乏语义理解以及难以实现低延迟流式推理的挑战。
method: 提出一种结合高阶运动建模、LSeg语义蒸馏和窗口因果注意力机制的统一前馈框架。
result: "实验表明，该方法在运动精度上提升21%，重建PSNR提高1.6 dB，语义分割mIoU提升20%。"
conclusion: SLARM通过语义与几何的深度融合，实现了高效、准确且支持自然语言查询的动态场景实时重建。
---
