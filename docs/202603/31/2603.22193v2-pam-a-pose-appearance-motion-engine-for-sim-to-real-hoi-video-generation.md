...
tags: ["query:q8"]
score: 7.0
evidence: 用于具身智能的虚实迁移HOI视频生成
tldr: 本研究针对手物交互（HOI）生成领域中姿态、外观和运动研究割裂的问题，提出了PAM（姿态-外观-运动）统一引擎。PAM能够实现可控的HOI视频生成，无需真实首帧即可完成从模拟到真实的转换。通过在DexYCB和OAKINK2数据集上的实验，PAM在视频质量和姿态准确性上均优于现有基准，并能通过合成数据显著提升下游手部姿态估计任务的性能。
selection_source: fresh_fetch
motivation: 现有的HOI生成研究在姿态合成、单图生成和视频生成方面相互孤立，且往往依赖真实首帧，限制了其在模拟到真实场景中的应用。
method: 提出了一种名为PAM的统一引擎，通过整合深度、分割和关键点等多种输入条件，实现连贯且可控的手物交互视频生成。
result: 在DexYCB数据集上，PAM显著降低了FVD和MPJPE指标，并能生成比基准模型分辨率更高的视频。
conclusion: PAM通过统一姿态、外观和运动，为高质量HOI视频生成提供了有效框架，并证明了合成数据在增强下游视觉任务中的巨大潜力。
---
