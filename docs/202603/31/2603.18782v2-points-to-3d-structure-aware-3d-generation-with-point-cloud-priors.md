...
tldr: 本研究提出 Points-to-3D 框架，旨在利用点云先验（如 LiDAR 采集或单图预测）来增强 3D 生成的几何可控性。该框架基于 TRELLIS 潜空间扩散模型，通过点云初始化结构潜变量，并结合结构补全网络与分阶段采样策略，实现了在保留输入可见区域的同时补全全局几何。实验证明，该方法在物体和场景生成任务中均优于现有基准，显著提升了渲染质量和几何保真度。
selection_source: carryover_cache
motivation: 现有的 3D 生成模型多依赖文本或图像，未能充分利用传感器或预测器提供的显式点云几何约束。
method: 基于 TRELLIS 框架，利用点云先验初始化潜变量，并采用结构补全网络及分阶段采样策略生成 3D 资产。
result: 在物体和场景生成实验中，该方法在渲染质量和几何一致性方面均显著超越了最先进的基准模型。
conclusion: 显式嵌入点云先验能有效提升 3D 生成的结构可控性与准确性，为高质量 3D 内容创作提供了新思路。
---

## 摘要
近期 3D 生成的进展主要由以图像或文本为条件的模型驱动，而现成的 3D 先验仍未得到充分利用。在许多现实场景中，可见区域的点云很容易通过 LiDAR 等主动传感器或 VGGT 等前馈预测器获得，它们提供了当前方法未能利用的显式几何约束。在这项工作中，我们介绍了 Points-to-3D，这是一个基于扩散的框架，利用点云先验进行几何可控的 3D 资产和场景生成。Points-to-3D 构建在潜空间 3D 扩散模型 TRELLIS 之上，首先将纯噪声的稀疏结构潜变量初始化替换为针对点云先验定制的输入公式。随后，在 TRELLIS 框架内使用旨在学习全局结构修复的特定任务数据训练的结构修复网络，配合阶段性采样策略（结构修复后进行边界细化）进行推理，在保留输入先验可见区域的同时补全全局几何。在实践中，Points-to-3D 可以接受精确的点云先验或从单张图像中由 VGGT 估计的点云作为输入。在物体和场景方案上的实验一致表明，在渲染质量和几何保真度方面，该方法优于最先进的基准模型，突显了显式嵌入点云先验对于实现更准确且结构可控的 3D 生成的有效性。
