...
tldr: Points-to-3D 是一种基于扩散模型的 3D 生成框架，旨在利用点云先验（如 LiDAR 采集或单图预测）来增强几何可控性。该方法基于 TRELLIS 模型，通过将点云先验融入潜在空间初始化，并结合结构修复网络与分阶段采样策略，实现了在保留输入可见区域的同时补全全局几何。实验证明，该方法在物体和场景生成任务中均优于现有基准，显著提升了渲染质量和几何保真度。
selection_source: carryover_cache
motivation: 现有的 3D 生成模型多依赖文本或图像，未能充分利用易于获取且能提供显式几何约束的点云先验。
method: 基于 TRELLIS 框架，通过点云先验初始化潜在结构，并利用结构修复网络与分阶段采样策略生成完整 3D 资产。
result: 在物体和场景生成实验中，该方法在渲染质量和几何保真度方面均显著优于当前最先进的基线模型。
conclusion: 显式嵌入点云先验能有效实现更精确且几何可控的 3D 生成，为结构感知的 3D 资产创作提供了新思路。
---
