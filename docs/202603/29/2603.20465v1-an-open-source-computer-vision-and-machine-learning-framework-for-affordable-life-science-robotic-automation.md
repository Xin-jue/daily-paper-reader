...
method: 集成了用于微生物图像语义分割的U-net模型和用于机械臂逆运动学预测的混合密度网络（MDN）。
result: 系统实现了小于1毫米的位置误差和小于4度的关节角预测误差，菌落检测的IoU达到0.537。
conclusion: 该开源框架证明了利用低成本硬件和先进算法实现高精度实验室自动化任务的可行性。
---

## 摘要
我们提出了一种开源机器人框架，该框架集成了计算机视觉和基于机器学习的逆运动学，以实现菌落挑选和液体处理等低成本实验室自动化任务。该系统使用定制训练的 U-net 模型对微生物培养物进行语义分割，并结合混合密度网络（Mixture Density Network）来预测简单的 5 自由度（5-DOF）机器人手臂的关节角度。我们使用经过改装并升级了定制液体处理末端执行器的机器人手臂对该框架进行了评估。实验结果证明了该框架在精确、可重复操作方面的可行性，其平均位置误差低于 1 mm，关节角度预测误差低于 4 度，菌落检测能力的 IoU 分数为 0.537，Dice 系数为 0.596。
