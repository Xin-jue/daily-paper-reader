...
tldr: 针对视觉语言模型（VLM）在机器人感知中难以准确推断物体质量的问题，本文提出了 PhysQuantAgent 框架和 VisPhysQuant 基准数据集。该框架通过引入目标检测、尺度估计和横截面图像生成三种视觉提示方法，增强了模型对物体尺寸和内部结构的理解。实验证明，这些方法显著提升了 VLM 在真实场景下的质量估计精度，为物理属性推理提供了新思路。
selection_source: carryover_cache
motivation: 现有的视觉语言模型在推断物体质量等物理属性方面能力有限，且缺乏针对真实感知条件下物理量估计的评估基准。
method: 提出了 PhysQuantAgent 框架，利用目标检测、尺度估计和横截面生成等视觉提示技术来辅助 VLM 理解物体的空间尺寸和内部结构。
result: 实验结果表明，所提出的视觉提示方法在真实世界数据集上显著提高了 VLM 的质量估计准确性。
conclusion: 将空间推理与 VLM 的先验知识相结合，是提升模型在物理属性推理任务中表现的有效途径。
---
