...
motivation: 旨在解决移动机器人在视觉导航中面临的场景迁移能力弱、计算资源有限和传感器成本高之间的三难困境。
method: 提出一种知识蒸馏框架，使轻量级单目学生网络在模仿全景深度专家网络动作的同时，与其潜在特征嵌入进行对齐。
result: 实验证明，该方法相比仅模仿动作的策略，显著增强了单目策略在不同场景下的迁移能力和导航表现。
